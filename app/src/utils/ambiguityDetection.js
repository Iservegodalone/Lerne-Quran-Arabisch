...
// ===== Buckwalter transliteration (not used for detection, but kept for reference) =====
// const BW_MAP = { ... }

// Tashkil regex from arabic.js + U+0670 (Alif Khanjariyya) for full consonantal extraction
...
function toConsonantal(text) {
  if (!text) return ''
  return text.replace(VOWEL_MARKS_REGEX, '').replace(/\u0640/g, '') // remove tatweel too
}

// ===== Active/Passive verb form pairs =====
// These verb forms produce IDENTICAL consonantal skeletons when unvowelled.
// Each entry lists the form and the two voices that collapse.
const ACTIVE_PASSIVE_COLLAPSE_PATTERNS = [
  // Form I perfect: fa'ala vs fu'ila => identical consonants
  { form: 'I', tense: 'perfect', note: 'fa\'ala (active) vs fu\'ila (passive) - identical rasm' },
  // Form I imperfect: yaf'alu vs yuf'alu => identical consonants
  { form: 'I', tense: 'imperfect', note: 'yaf\'alu (active) vs yuf\'alu (passive) - identical rasm' },
  // Form II perfect: fa''ala vs fu''ila
  { form: 'II', tense: 'perfect', note: 'fa\'\'ala (active) vs fu\'\'ila (passive) - identical rasm' },
  // Form III perfect: faa'ala vs fuu'ila
  { form: 'III', tense: 'perfect', note: 'faa\'ala (active) vs fuu\'ila (passive) - identical rasm' },
  // Form IV perfect: af'ala vs uf'ila
  { form: 'IV', tense: 'perfect', note: 'af\'ala (active) vs uf\'ila (passive) - identical rasm' },
  // Form IV imperfect: yuf'ilu vs yuf'alu
  { form: 'IV', tense: 'imperfect', note: 'yuf\'ilu (active) vs yuf\'alu (passive) - identical rasm' },
]
