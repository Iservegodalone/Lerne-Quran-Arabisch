...
function Step1ParticleMarking({ words, onComplete, surah, verse }) {
  ...
  // Detect Huruf Muqatta'at (INL = initial letters) among non-particle words
  ...
  const handleContinue = useCallback(() => {
    // words to analyze = those NOT in groundTruth.all and NOT Huruf Muqatta'at (INL)
        ...
        {ambiguity && ambiguity.options && ambiguity.options.length > 1 && (
          <div style={{ marginTop: '10px', padding: '10px', background: 'var(--ambiguous-bg)', borderRadius: 'var(--radius)', border: '1px solid var(--ambiguous)' }}>
            ...
            <p style={{ fontSize: '0.78rem', color: 'var(--text-muted)', marginTop: '4px' }}>
              Alle genannten Optionen sind grammatisch gueltig.
            </p>
          </div>
        )}
      </div>
...
function WordAnalysisPanel({
  ...
  const handleCheckRoot = () => {
    ...
    if (!isPlausibleRoot(parsed)) {
      setRootFeedback({ type: 'error', msg: 'Bitte gib gueltige arabische Konsonanten ein (3-4 Buchstaben).' })
      return
    }
  ...
  const handleCheckForm = useCallback(() => {
    if (!wordType) {
      setFormFeedback({ type: 'error', msg: 'Bitte waehle den Worttyp.' })
      return
    }
      ...
      <div style={{ ...S.card, opacity: subStep === 'root' || rootVerified ? 1 : 0.5, pointerEvents: subStep === 'root' ? 'auto' : (rootVerified ? 'auto' : 'none') }}>
        ...
        {!rootVerified && (
          <>
            ...
            {/* Stage 3: Flagging prompt — "Moechtest du sie als Alternative vorschlagen?" */}
            ...
            {flagSaved && (
              <div style={{ ...S.feedback('correct'), marginTop: '8px', fontSize: '0.83rem' }}>
                Alternative gespeichert! Sie wird kuenftig bei diesem Wort angezeigt.
              </div>
            )}
        ...
        {subStep === 'meaning' && (
          <>
            ...
            <div style={{ ...S.flexRow, justifyContent: 'flex-end' }}>
              ...
              >
                {phase >= 3 ? 'Weiter zur Syntax' : 'Wort abschliessen'}
              </button>
            </div>
          </>
        )}
              ...
              {syntaxRole && (() => {
                ...
                return (
                  <div style={{
                    ...
                    {/* Idafa-Subtypes: show when mudaf_ilayh is selected */}
                    {role.value === 'mudaf_ilayh' && idafaSubtypes?.subtypes && (
                      <div style={{ marginTop: '8px', paddingTop: '6px', borderTop: '1px solid var(--border)', fontSize: '0.75rem' }}>
                        <div style={{ fontWeight: 600, marginBottom: '3px', color: 'var(--text-primary)' }}>Idafa-Typen (semantisch):</div>
    ...
    <div className="m3-container" style={S.container}>
      ...
      <div style={S.workPanel}>
        ...
        <div style={S.workScrollArea}>
          ...
          {step === 5 && verse && (
            <div style={S.card}>
              <div style={S.cardTitle}>Schritt 5: Rasm-Leseuebung (optional)</div>
              <p style={S.muted}>
                Uebe das Lesen des Konsonantentextes in verschiedenen Schichten — von voll punktiert bis zum reinen Rasm.
              </p>
