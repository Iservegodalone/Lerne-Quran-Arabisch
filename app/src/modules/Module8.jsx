...
function AnnotationsView({ onBack }) {
  ...
  return (
    <div style={{ maxWidth: '900px', margin: '0 auto' }}>
      ...
      {notes.length === 0 && <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem' }}>Noch keine Notizen. Fuege oben eine hinzu.</p>}
      {notes.map(a => {
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px' }}>
              ...
              <div style={{ display: 'flex', gap: '6px' }}>
                ...
                <button onClick={() => handleDelete(a.surah, a.verse)}
                  style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '0.8rem', color: 'var(--incorrect)' }}>Loeschen</button>
              </div>
            </div>
            {editKey === key ? (
              <div style={{ display: 'flex', gap: '6px' }}>
...
function LayerBuildupDrill({ onBack }) {
  ...
  const steps = [
    ...
    { label: 'I\'jam', desc: 'Punkte hinzugefuegt — Konsonanten eindeutig', text: ex.simple, color: '#f39c12' },
...
function LoanwordBrowser({ onBack }) {
  ...
  return (
    <div style={{ maxWidth: '900px', margin: '0 auto' }}>
      ...
      <p style={{ color: 'var(--text-secondary)', marginBottom: '16px', fontSize: '0.9rem' }}>
        {loanwordData.meta?.description || 'Sprachwissenschaftlich anerkannte Lehnwoerter.'} ({loanwordData.meta?.totalEntries || entries.length} Eintraege)
      </p>
...
export default function Module8() {
  ...
  // ===== P3: Huruf Muqatta'at Reference =====
  if (view === VIEWS.MUQATTAAT_REF) {
    const muqData = alphabetData?.hurufMuqattaat || []
    return (
      <div style={{ maxWidth: '900px', margin: '0 auto' }}>
        {newDrillBack}
        <h2 style={{ marginBottom: '8px' }}>Huruf Muqatta'at — Einzelbuchstaben am Surenanfang</h2>
        <p style={{ color: 'var(--text-secondary)', marginBottom: '16px', fontSize: '0.9rem' }}>
      ...
      <section style={{ marginBottom: '28px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '12px' }}>
          ...
          <button onClick={() => setView(VIEWS.LOANWORDS)} style={{
            ...
            <div style={{ fontWeight: 600, marginBottom: '4px' }}>Lehnwoerter-Verzeichnis</div>
            <div style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>
              {loanwordData.meta?.totalEntries || '?'} sprachwissenschaftlich anerkannte Lehnwoerter mit Herkunftssprache, Etymologie und koranischer Verwendung.
            </div>
          </button>
      ...
      <section style={{ marginBottom: '28px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '12px' }}>
          ...
          <button onClick={() => setView(VIEWS.MUQATTAAT_REF)} style={{ padding: '20px', textAlign: 'left', borderRadius: 'var(--radius-lg)', background: 'var(--bg-card)', border: '1px solid var(--border)', cursor: 'pointer', color: 'var(--text-primary)' }}>
            ...
            <div style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>Alle 14 Einzelbuchstaben in 29 Suren — Referenzuebersicht mit Verteilung und Statistik.</div>
          </button>
          <button onClick={() => setView(VIEWS.MUSHAF_NOTATION)} style={{ padding: '20px', textAlign: 'left', borderRadius: 'var(--radius-lg)', background: 'var(--bg-card)', border: '1px solid var(--border)', cursor: 'pointer', color: 'var(--text-primary)' }}>
            ...
            <div style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>Gliederungszeichen: Juz, Hizb, Rub, Basmala und Versend-Zeichen.</div>
          </button>
      ...
      <section style={{ marginBottom: '28px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '12px' }}>
          <button onClick={() => setView(VIEWS.WRITING)} style={{
            ...
            <div style={{ fontWeight: 600, marginBottom: '4px' }}>Schreibuebung</div>
            <div style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>
          ...
          <button onClick={() => setView(VIEWS.PRINT)} style={{
            ...
            <div style={{ fontWeight: 600, marginBottom: '4px' }}>Druckbare Referenzblaetter</div>
            <div style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>
      ...
      <section style={{ marginBottom: '32px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '12px' }}>
          ...
          {HamzaExercise && <button onClick={() => setView(VIEWS.HAMZA)} style={{
            ...
            <div style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>Hamza-Sitz, Wasl vs. Qat, Schreibregeln systematisch ueben.</div>
          </button>}
      ...
      <section style={{ marginBottom: '28px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '12px' }}>
          ...
          <button onClick={() => setView(VIEWS.MUSHAF_NOTATION)} style={{
            ...
            <div style={{ fontWeight: 600, marginBottom: '4px' }}>Mushaf-Notation</div>
            <div style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>Juz, Hizb, Rubu und andere Markierungen im Mushaf.</div>
          </button>
        </div>
      </section>
