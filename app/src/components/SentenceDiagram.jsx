...
function validateAnalysis(assignments, words) {
  ...
  // 1. Verbal sentence: verb without Fa'il
  ...
  // 4c. Maf'ul Mutlaq requires a verb
...
function buildSyntaxTree(assignments, words) {
  ...
  // Group: Verb -> Fa'il, Maf'ul bihi, Hal, Tamyiz, Maf'ul mutlaq as children
  const verbs = entries.filter(e => e.roleId === 'fi3l')
  verbs.forEach(verb => {
    used.add(verb.idx)
    const children = []

    // Find Fa'il / Na'ib al-Fa'il
    ...
    // Find Hal, Tamyiz, Maf'ul mutlaq, Zarf
