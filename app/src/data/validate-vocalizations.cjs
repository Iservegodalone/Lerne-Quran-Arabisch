...
function loadVocalizedQuran() {
  let filePath = VOCALIZED_FILE;
  if (!fs.existsSync(filePath)) {
    filePath = UTHMANI_FILE;
  }
  if (!fs.existsSync(filePath)) {
    console.error('ERROR: Neither quran-vocalized.json nor quran-uthmani.json found.');
    process.exit(1);
  }
